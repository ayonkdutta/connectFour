...
class Main {
    ...
    public static boolean closeOne() { //checks for comp if you have 3 in a row
        if (column == -1) {
            return false;
        } else {
            for (int i = 0; i < board.length; i++) {
                for (int j = 2; j < board[0].length; j++) {
                    if (board[i][j].equals("○") // horizontal
            ...
            for (int i = 0; i < board.length - 2; i++) { // diagonal backwards
            ...
            for (int i = board.length - 2; i > 2; i--) { // diagonal forwards
                for (int j = 2; j < board[0].length - 2; j++) {
                    if (board[i][j].equals("○")
                            && board[i][j].equals(board[i - 1][j + 1])
                            && board[i][j].equals(board[i - 2][j + 2])
                            && !board[i - 2][j + 3].equals(" ")) {
                        column = j + 3;
                        return true;
                    }
                }
